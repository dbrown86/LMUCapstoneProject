...
class DataLoader:
    ...
    def load_donors(self):
        ...
        try:
            # Try parquet first (newer format)
            parquet_path = self.data_dir / 'parquet_export' / 'donors_enhanced_phase1.parquet'
            if parquet_path.exists():
                donors_df = pd.read_parquet(parquet_path)
                return donors_df
            ...
            raise FileNotFoundError(f"Donor data not found at {parquet_path} or {donors_path}")
